...
class VendorCartState {
  ...
  double get subtotal {
    ...
    for (final item in items) {
      final svc = services.firstWhere((s) => s.id == item.serviceId, orElse: () => const ServiceModel(id: '', vendorId: '', name: '', description: '', category: ServiceCategory.wash, minWeightKg: 0));
...
class VendorCartNotifier extends StateNotifier<VendorCartState> {
  VendorCartNotifier(this._repo) : super(const VendorCartState(items: [], services: []));
  ...
  Future<void> init(String vendorId) async {
    ...
    final filteredItems = cart.items.where((i) => svcIds.contains(i.serviceId)).toList();
  ...
  void updateQuantity(String serviceId, int delta) {
    ...
    if (idx >= 0) {
      ...
      if (newQty <= 0) {
        ...
        // Async update in storage
        _repo.getCart().then((c) {
          final item = c.items.firstWhere((i) => i.serviceId == serviceId, orElse: () => const CartItem(id: '', serviceId: '', quantity: 0));
          if (item.id.isNotEmpty) _repo.removeFromCart(item.id);
        });
      } else {
        ...
        _repo.getCart().then((c) {
          ...
          if (item.id.isNotEmpty) {
            _repo.updateCartItem(cartItemId: item.id, quantity: newQty);
          }
        });
      }
    } else if (delta > 0) {
      ...
      _repo.addToCart(serviceId: serviceId, quantity: delta);
    }
...
final vendorCartProvider =
    StateNotifierProvider.autoDispose<VendorCartNotifier, VendorCartState>((ref) {
  final repo = ref.watch(customerRepositoryProvider);
  return VendorCartNotifier(repo);
});
